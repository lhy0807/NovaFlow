    ...
    def __init__(self, resolution: Tuple[int, int], output_dim: int, stride: int, pretrained: bool, freeze_mode: str):
        ...
        if pretrained:
            try:
                ...
                logger.warning(f"Failed to load CoTracker from hub, trying force_reload=True: {e}")
                cotracker = torch.hub.load("facebookresearch/co-tracker", "cotracker3_offline", force_reload=True)
