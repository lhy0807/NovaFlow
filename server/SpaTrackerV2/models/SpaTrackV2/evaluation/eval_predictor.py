...
from models.SpaTrackV2.datasets.delta_utils import (
    bilinear_sample2d,
    bilinear_sampler,
    get_grid,
...
class EvaluationPredictor(torch.nn.Module):
    ...
    def forward(
        ...
        if is_sparse:
            if self.front_model is not None:
                ...
                video_tensor = preprocess_image(video[0])[None]
                with torch.no_grad():
                    with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                        # Predict attributes including cameras, depth maps, and point maps.
                        predictions = self.front_model(video.cuda()/255)
                        extrinsic, intrinsic = predictions["poses_pred"], predictions["intrs"]
                        depth_map, depth_conf = predictions["points_map"][..., 2], predictions["unc_metric"]
                    depth_tensor = depth_map.squeeze().cpu().numpy()
                    extrs = np.eye(4)[None].repeat(len(depth_tensor), axis=0)
                    extrs = extrinsic.squeeze().cpu().numpy()
                    intrs_vggt = intrinsic.squeeze().cpu().numpy()
                ...
                if viz:
                    if out_dir is None:
                        out_dir = f"/mnt/bn/xyxdata/home/codes/my_projs/SpaTrack2/vis_results/test"
                        os.makedirs(out_dir, exist_ok=True)
                    for i in range(extrs.shape[0]):
                        img = (video_tensor)[i].permute(1,2,0).cpu().numpy()
                        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                        cv2.imwrite(os.path.join(out_dir, f'frame_{i:04d}.png'), img)
                        depth_i = depth_tensor[i]
                        np.save(os.path.join(out_dir, f'frame_{i:04d}.npy'), depth_i)

                    np.save(os.path.join(out_dir, f'intrinsics.npy'), intrs_vggt)
                    np.save(os.path.join(out_dir, f'extrinsics.npy'), extrs)  
                    np.save(os.path.join(out_dir, f'conf.npy'), unc_metric) 
                    import pdb; pdb.set_trace()
                videodepth = torch.from_numpy(depth_tensor[None,:,None])
                depth_init = torch.from_numpy(depth_tensor[:1][None])
                intrs = torch.from_numpy(intrs_vggt[None])
                extrs = torch.from_numpy(extrs[None])
                extrs = torch.eye(4)[None].repeat(len(depth_tensor),1,1)[None]
                print("success")
    ...
    def forward_sparse(
        self, video, videodepth, queries, depth_init, intrs=None, return_3d=False, lift_3d=False, use_2d_only=False, extrs=None
    ):
        ...
        if use_2d_only:
            ...
            with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16):
                (
                    c2w_traj, _, point_map, conf_depth,
                    track3d_pred, track2d_pred, vis_pred, conf_pred, video
                ) = self.model.forward(video[0], depth=videodepth[0,:,0],
                                    intrs=intrs[0], extrs=None if extrs is None else extrs[0], 
                                    queries=queries[0, ..., :3].cpu().numpy(),
                                    fps=1, full_point=False, queries_3d=queries[0].cpu().numpy(),
                                    query_no_BA=True, fixed_cam=True, stage=1, support_frame=len(video)-1, replace_ratio=1.0) 

            traj_e, traj_d_e, vis_e = (
                track2d_pred[..., :2][None],
                track2d_pred[..., 2:3][None],
                vis_pred[None,...,0],
            )
