...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="Generate multiple images or videos from a text prompt or image using Wan"
    )
    parser.add_argument(
        "--task",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The base file name to save the generated images or videos to. Index will be appended.")
    parser.add_argument(
        "--number",
        type=int,
        default=10,
        help="Number of videos/images to generate.")
    parser.add_argument(
        "--src_video",
        type=str,
        default=None,
...
def generate_multiply(args):
    ...
    # Initialize model and inputs once
    model = None
    img = None
    first_frame = None
    last_frame = None
        ...
        if args.use_prompt_extend:
            ...
            if rank == 0:
                prompt_output = prompt_expander(
                    ...
                    seed=args.base_seed)
                if hasattr(prompt_output, 'status') and prompt_output.status == False:
                    logging.info(
                        f"Extending prompt failed: {prompt_output.message}")
                    logging.info("Falling back to original prompt.")
                    input_prompt = args.prompt
                else:
                    input_prompt = prompt_output.prompt if hasattr(prompt_output, 'prompt') else args.prompt
    ...
    # Generate multiple videos/images
    for i in range(args.number):
        current_seed = args.base_seed + i
        logging.info(f"Generating {'image' if 't2i' in args.task else 'video'} {i+1}/{args.number} with seed {current_seed}...")
        ...
        if rank == 0:
            # Generate save file name with index
            if args.save_file is None:
                formatted_time = datetime.now().strftime("%Y%m%d_%H%M%S")
                formatted_prompt = args.prompt.replace(" ", "_").replace("/", "_")[:50] if args.prompt else "prompt"
                suffix = '.png' if "t2i" in args.task else '.mp4'
                base_name = f"{args.task}_{args.size.replace('*','x') if sys.platform=='win32' else args.size}_{args.ulysses_size}_{args.ring_size}_{formatted_prompt}_{formatted_time}"
                save_file = f"{base_name}_{i}{suffix}"
            else:
                # Insert index before file extension
                if "t2i" in args.task:
                    if args.save_file.endswith('.png'):
                        save_file = args.save_file[:-4] + f"_{i}.png"
                    else:
                        save_file = f"{args.save_file}_{i}.png"
                else:
                    if args.save_file.endswith('.mp4'):
                        save_file = args.save_file[:-4] + f"_{i}.mp4"
                    else:
                        save_file = f"{args.save_file}_{i}.mp4"
            breakpoint()
            if "t2i" in args.task:
    ...
    logging.info(f"Finished generating {args.number} {'images' if 't2i' in args.task else 'videos'}.")


if __name__ == "__main__":
    args = _parse_args()
    generate_multiply(args)
